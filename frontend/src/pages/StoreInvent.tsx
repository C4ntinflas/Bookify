...
const StoreInvent = () => {
  ...
  const storeId = (location.state as any)?.store_id;

  const [store, setStore] = useState<Book[]>([]);
  ...
  useEffect(() => {
    if (storeId) {
      setLoading(true);
      axios
        .get<Book[]>(`http://localhost:3001/books/store/${storeId}`)
        .then((response) => {
            ...
            <tbody>
              {store.map((book, index) => (
                <tr key={book.book_id} className='text-center'>
                  ...
                  <td className='border py-2 px-4 hidden md:table-cell'>{book.quantity}</td>
